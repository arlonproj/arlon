...
if ! argocd &>/dev/null; then
  ...
  curl -sSL -o "${HOME}/.local/bin/argocd" "https://github.com/argoproj/argo-cd/releases/download/v2.4.18/argocd-${os}-${arch}"
...
#clusterawsadm bootstrap iam create-cloudformation-stack is not required as we are using a cloudformation template that was already created
