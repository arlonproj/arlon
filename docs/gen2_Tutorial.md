...
Before deploying a EKS cluster, make sure to setup the AWS Environment as stated here: <https://cluster-api.sigs.k8s.io/user/quick-start.html>
...
Before deploying a EKS managed node group cluster using AWSManagedMachinePool, make sure to setup the AWS Environment as stated here: <https://cluster-api.sigs.k8s.io/user/quick-start.html>
In addition to this, export the following environment variable.
```shell
export EXP_MACHINE_POOL=true
```
Then initialize the environment as stated [here](https://cluster-api-aws.sigs.k8s.io/topics/machinepools.html#awsmanagedmachinepool)
...
```
--feature-gates=EKS=true,EKSEnableIAM=false,EKSAllowAddRoles=false,EKSFargate=true,MachinePool=true,EventBridgeInstanceState=false,AutoControllerIdentityCreator=true,BootstrapFormatIgnition=false,ExternalResourceGC=false
```
