...
* Profile: any unique label value that appears in the spec.generators[0].clusters.selector.matchExpressions.values[]
...
by applications. If a particular profile label value is not referenced from
any application, it does not exist.0
...
referenced in the matchExpressions.values[] arrays.
